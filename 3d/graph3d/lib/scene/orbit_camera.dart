...
class OrbitCamera extends ChangeNotifier {
  ...
  /// Where the original parks its camera, whatever the window. Fitting a
  /// 4200-unit-wide table into a portrait phone would need almost twice this,
  /// shrinking every card to an unreadable speck, so this is the floor on how
  /// far back framing may go. Panning is tethered to the layout, so the part
  /// that spills off a narrow screen can always be brought back.
  static const double _defaultDistance = 8000;
  ...
  double fitDistance(Vector3 halfExtent) {
    ...
    return math.min(fit, _defaultDistance).clamp(minDistance, maxDistance);
  }
  ...
  void frame(
    ...
    final target = distance ??
        (halfExtent == null ? _defaultDistance : fitDistance(halfExtent));
    _flyTo(
      target: center,
